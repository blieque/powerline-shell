    ...
    if not powerline.args.cwd_only:
        for n in names[:-1]:
            if n == '~' and Color.HOME_SPECIAL_DISPLAY:
                powerline.append(n, Color.HOME_FG, Color.HOME_BG)
            else:
                powerline.append(n, Color.PATH_FG, Color.PATH_BG,
                                 powerline.separator_thin, Color.SEPARATOR_FG)

    if names[-1] == '~' and Color.HOME_SPECIAL_DISPLAY:
        powerline.append(names[-1], Color.HOME_FG, Color.HOME_BG)

    else:
        powerline.append(names[-1], Color.CWD_FG, Color.PATH_BG)
